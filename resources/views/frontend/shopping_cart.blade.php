    ...
    <div class="container">
        ...
        <div class="one_half_less last">
            <div class="alertymes7">
                ...
                <div class="pull-right red"><b id="discount"></b></div>
                <div class="clearfix"></div>
                ...
                <input type="hidden" name="discount" id="discount_submit" value="">
        ...
        $(function() {
            ...
            var table = $('.data-table').DataTable({
                ...
                drawCallback: function(data) {
                    ...
                    $("#discount").html('RM ' + data.json.discount);
                    $("#grandTotal").html('RM ' + data.json.grandTotal);

                    $("#total_price").val((data.json.total).replace(/,/g, ''));
                    $("#discount_submit").val((data.json.discount).replace(/,/g, ''));
            ...
            $('body').on('click', '.servicePrice', function(e) {
                e.preventDefault();
                var itemId = $(this).data('id');
                let index = $(".servicePrice").index(this);
            ...
            $('body').on('click', '.domainPrice', function(e) {
                e.preventDefault();
                var itemId = $(this).data('id');
                let index = $(".domainPrice").index(this);
                currentCycleValue = $('.cycle_value').eq(index).text().charAt(0);
                $('#json_message_modal').html('');
                //toastr.error('click on domain price ', 'error');

                $.ajax({
                    url: base_url + '/get_tld_price',
                    type: 'POST',
                    data: {
                        '_token': csrf_token,
                        'tld': itemId
                    ...
                    success: function(response) {
                        ...
                        $.each(response, function(key, value) {
                            ...
                            if (key == 0) {
                                var price = (key + 1) + ' year @ RM ' + value;
                            } else {
                                var price = (key + 1) + ' years @ RM ' + value;
                            }
                            if (value != 0) {
                                $("#cycle").append('<option value="' + price + '">' + price + '</option>');
                            }
                        });
                        $("#id").val(itemId);
                        $('#modalTitle').html("Edit Cycle");
            ...
            // $('body').on('click', '.domainPrice', function() {

            //     var itemId = $(this).data('id');
            //     let index = $(".domainPrice").index(this);
            //     currentCycleValue = $('.cycle_value').eq(index).text().charAt(0);
            //     $('#json_message_modal').html('');
            //     var url = base_url + '/get_tld_price';
            //     $.ajax({
            //         url: url,
            //         type: 'POST',
            //         data: {
            ...
            $('#emptyCart').click(function() {
                ...
                var con = confirm("Are You sure want to delete all items !");
                if (con) {
                    $.ajax({
                        url: base_url + '/empty_cart',
                        type: 'GET',

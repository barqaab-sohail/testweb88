    ...
    <div class="three_fourth_less last">
      ...
      <div class="one_half_less last">
        <div class="alertymes6">
          <h3 class="light"><i class="fa fa-warning"></i><strong>You have {{$expiredDomain}} domain(s) expiring within the next 30 days.</strong>
            <br />Renew and secure them today for peace of mind.
          </h3>
          <div class="clearfix margin_bottom1"></div>
          <a href="{{ url('domain_domain_renewal') }}" class="btn btn-warning caps"><i class="fa fa-refresh"></i><b>Renew Now</b></a>
        </div><!-- end section -->
      </div><!-- end one half less -->
      <div class="clearfix margin_bottom3"></div>

      <h4>Overdue Invoices</h4>
      <p class="aliright">{{$unpaidOrders->total()}} record(s) found. Items {{($unpaidOrders->currentpage()-1)*$unpaidOrders->perpage()+1}}-{{$unpaidOrders->currentpage()*$unpaidOrders->perpage()}} out of {{$unpaidOrders->total()}} displayed</p>
      ...
      <div class="table-responsive">
        ...
        <table class="table table-hover table-striped">
          ...
          <tbody>
            ...
            @php $i = ($unpaidOrders->perPage() * ($unpaidOrders->currentPage() - 1)) + 1; @endphp
            @foreach($unpaidOrders as $key => $value)
            <tr>
              <td>{{$i++}}</td>
              ...
              <td>RM {{ number_format($value->total_amount + $value->gst_rate, 2) }}</td>
              <td>
        ...
        @if ($unpaidOrders->lastPage() > 1)
        <div class="pagination center">
          <b>Page {{ $unpaidOrders->currentPage() }} of {{ $unpaidOrders->lastPage() }}</b>

          <a href="{{ $unpaidOrders->url($unpaidOrders->currentPage()-1) }}" class="navlinks {{ ($unpaidOrders->currentPage() == 1) ? ' disabled' : '' }}">&lt; Previous</a>

          @for ($i = 1; $i <= $unpaidOrders->lastPage(); $i++)
            <a class="navlinks {{ ($unpaidOrders->currentPage() == $i) ? ' current' : '' }}" href="{{ $unpaidOrders->url($i) }}">{{ $i }}</a>
            @endfor

            <a href="{{ $unpaidOrders->url($unpaidOrders->currentPage()+1) }}" class="navlinks">Next ></a>
        </div>
        @endif
      </div>
      ...
      <div class="alicent">
        <a href="{{ url('billing_mass_payment') }}" class="btn btn-danger caps"><i class="fa fa-mouse-pointer"></i> <b>Make Payment</b></a>&nbsp;
      </div>
      ...
      <div class="table-responsive">
        ...
        <table class="table table-hover table-striped">
          ...
          <tbody>
            ...
            <tr>
              <td>{{ $i}}</td>
              <td><a href="{{url('/order-details/'.$order->id)}}">MY-{{ $order->transaction_id}}</a></td>

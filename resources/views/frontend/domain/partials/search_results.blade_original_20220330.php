                    ...
                    <thead>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <select class="form-control input-medium" id="domain-search-dropdown" onchange="getDataValue()">
                                        @foreach(json_decode($findedDomainPrice->pricing) as $price)
                                            <option value="{{$loop->index + 1}}|{{$price->s}}" {{ $loop->index == 1 ? 'selected="selected"':''}}>{{$loop->index + 1}} year(s) @ RM {{$price->s}}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="btn-sm btn-danger caps add_to_cart"><i class="fa icon-basket-loaded"></i> <b>Add</b></a>&nbsp;

...
?>
                          ...
                          <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="form-horizontal" id="frm_pricing">
                                                ...
                                                <div class="form-group">
                                                    <label for="inputFirstName" class="col-md-3 control-label">Pricing Display <span class="text-red">*</span></label>
                                                    <div class="col-md-6">
                                                        <div class="xss-margin"></div>
                                                        <div class="radio-list">
                                                            <label><input name="hide_price" {{$plan->hide_on_mobile == 1 ? 'checked' : ''}}  id="hide_price1" type="radio" value="1" />&nbsp; Display in the frontend</label>
                                                            <label><input name="hide_price" {{$plan->hide_on_mobile == 0 ? 'checked' : ''}} id="hide_price" type="radio" value="0"/>&nbsp; Hide in the frontend</label>
            ...
            <tbody>
              <form id="delete_client">
                 {!!csrf_field()!!}
                                ...
                                <?php $count_plan_features++;?>
                                <tr>
                                  <td><input name="id[]" class="pflcheckboxes" value="{{$i->id}}" type="checkbox"/></td>
                                  <td>{{$count_plan_features}}</td>
                  <td><span class="label label-xs label-{{$i->status?'success':'danger'}}">{{$i->status?'Active':'Inactive'}}</span></td>
                  <td><div id="plf_title_{{$i->id}}">{{$i->title}}</div></td>
                  <td><a href="javascript:void" data-target="#modal-edit-plan-feature-{{$i->id}}" data-toggle="modal"  title="Edit"><span class="label label-sm label-success"><i class="fa fa-pencil"></i></span></a>
                   <a href="#" data-hover="tooltip" data-placement="top" title="Delete" data-target="#modal-delete-plan-feature-{{$i->id}}" data-toggle="modal"><span class="label label-sm label-red"><i class="fa fa-trash-o"></i></span></a>
                    <!--Modal Edit plan feature start-->
                      ...
                      <div class="row">
                        <div class="col-md-12">
                          ...
                          <input type="hidden" name="plan_id" id="" value="{{$plan->id}}">
                            <div class="form-group">
                              ...
                              <div class="col-md-8">
                                ...
                                <div class="radio-list">
                                  ...
                                  <input type="number" min="0" name="fee" id="service_free_domains_fee"  class="form-control" @if(isset($service_free_domains) && $service_free_domains->type==3) value="{{$service_free_domains->fee}}" @endif placeholder="0.00" value="0.00" step="0.01">
                            ...
                            <div class="form-actions">
                              <div class="col-md-offset-5 col-md-7"> <button class="btn btn-red" onclick="submit_data()">Save &nbsp;<i class="fa fa-floppy-o"></i></button>&nbsp; <a href="#" data-dismiss="modal" class="btn btn-green">Cancel &nbsp;<i class="glyphicon glyphicon-ban-circle"></i></a> </div>
                            </div>
   ...
   <div id="modal-delete-selected-pfd" tabindex="-1" role="dialog" aria-labelledby="modal-login-label" aria-hidden="true" class="modal fade">
				  ...
				  <div id="modal-delete-selected" tabindex="-1" role="dialog" aria-labelledby="modal-login-label" aria-hidden="true" class="modal fade">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<div class="modal-body" id="delete-selected-body">
										<div id="delete-selected-body-information-html"></div>
										<p id="selected_zero1" style="display:none" class="alert alert-danger">Please select aleast one item for delete</p>
										<div class="form-actions" id="delete-selected-buttons1">
											<div class="col-md-offset-4 col-md-8"> <button type="button" onclick="deleteSelectedPFL()" class="btn btn-red">Yes &nbsp;<i class="fa fa-check"></i></button>&nbsp; <a href="#" data-dismiss="modal" class="btn btn-green">No &nbsp;<i class="fa fa-times-circle"></i></a> </div>
										</div>
									</div>
								</div>
							</div>
    ...
    function open_modal_delete_selected() {
      $('#delete-selected-body-information-html').html("");
      $("#modal-delete-selected").modal('show');
      var selected=$('input[name="id[]"]:checked').length;
      if (selected<1) {
        $('#selected_zero1').show();
        $('#delete-selected-buttons1').hide();
      }else{
        $('#selected_zero1').hide();
        $('#delete-selected-buttons1').show();

      }
	    var titles = '';
		$(".pflcheckboxes").each(function(){
			if($(this).is(":checked")){
				var ids = $(this).val();
				var ttt = $("#plf_title_"+ids).html();
				titles = titles+'<strong>#'+ids+'</strong> '+ttt+'<br />';
			}
		});
		$("#delete-selected-body-information-html").html(titles);
    }
	function deleteSelectedPFL(){

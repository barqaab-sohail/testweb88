...
@section('custom_scripts')
    ...
    <script type="text/javascript">
        ...
        function delete_bulk(type) {
			var selected= document.getElementById(type); 
			console.log(selected);
			var delete_client_data={};
			$.each($(".id_ltd"), function(index, item){
				delete_client_data[index]=$(item).text();
			});
			//delete_client_data['_token']=csrf_token;
			console.log(delete_client_data);
            ...
            if ($(".id_ltd").length < 1) {
                ...
                $.ajax({
                    url: page_url + 'delete',
                    type: 'POST',
                    data: {id:delete_client_data, '_token': csrf_token}
                })
                    .done(function () {
                        location.reload();
                    })
                    ...
                    .done(function (response) {
                        ...
                        for (var i = 0; i < response.length; i++) {
                            $('#delete-selected-body-information').prepend('<p>' +
                                '<strong>#' + response[i].id + '</strong>:<span>' + response[i].tld + '</span>' +
                                '</p>');
                        }
                    })
                    .fail(function () {

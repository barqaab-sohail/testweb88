...
<div id="{{ $type }}" class="tab-pane fade in {{ $active }}">
	...
	<div class="portlet-header">
		...
		<li><a href="javascript:voi" onclick="open_modal_delete_selected('{{ $type }}')">Delete selected item(s)</a></li>
			<li class="divider"></li>
	...
	<div class="portlet-body">
		...
		@php
			...
			if($price_id != ''){
			$perPage = 1;
				$items = App\Models\DomainPricingList::where('id', $price_id)->where('domain_pricing_id', $item->id)->paginate($perPage);
			}
		@endphp
		...
		<form id="delete_client" class="{{ $type }}">
		...
		<table class="table table-hover table-striped">
			...
			<tr>
				<th width="1%"><input type="checkbox" data-toggler data-toggle=".toggle-list-{{ $type }}" onclick="all_check()" id="allcheck"></th>
				<th>#</th>
				<th>TLD</th>
				<th>1 Year</th>
				<th>2 Years</th>
              ...
              <div class="modal-content">
                ...
                <div class="modal-body" id="delete-selected-body">
                  ...
                  <div class="form-actions" id="delete-selected-buttons">
                    <div class="col-md-offset-4 col-md-8"> <button type="button" id="delete_bulk" class="btn btn-red" onclick="delete_bulk('{{ $type }}')">Yes &nbsp;<i class="fa fa-check"></i></button>&nbsp; <a href="#" data-dismiss="modal" class="btn btn-green">No &nbsp;<i class="fa fa-times-circle"></i></a> </div>
                  </div>
                </div>
              </div>
            </div>
...
function open_modal_delete_selected(type) {
	console.log(type);
$('#delete-selected-body-information').html("");
$("#modal-delete-selected").modal('show');
var selected= document.getElementById(type); 
console.log(selected);
var delete_client_data=$(selected).find("#delete_client").serialize();
selected=$(selected).find('input[name="id[]"]:checked').length;
console.log(selected);
if (selected<1) {
$('#selected_zero').show()
...
type: 'POST',
data: delete_client_data
})
.done(function(response) {
console.log(response);
for (var i=0; i <response.length; i++) {
$('#delete-selected-body-information').prepend('<p>'+
  '<strong>#'+'<span class="id_ltd">'+response[i].id+'<span>'+'</strong>:<span>'+response[i].tld+'</span>'+
'</p>');
}
})
.fail(function() {

...
//dd($order, $otp);
    ...
    public function getOrderToProductTax($order_id) {
        ...
        $orderToProduct->select('otp.*', 'p.*', 'c.title as color_name', 'se.event_type', 'se.token', 'rbd.date_checkin','rbd.date_checkout');
        ...
        $orderToProduct->leftjoin('room_booked_date as rbd', 'rbd.order_id', '=', 'otp.order_id'); 
        $orderToProduct->where('otp.order_id', $order_id);
        $orderToProduct->groupBy('otp.product_id');
    ...
    public function getOrderToProduct($order_id) {
        ...
        $orderToProduct->leftjoin('products as p', 'p.id', '=', 'otp.product_id');
        $orderToProduct->leftjoin('special_events as se', 'se.id', '=', 'otp.special_event_id');
        $orderToProduct->leftjoin('property as pr', 'pr.property_id', '=', 'p.property_id');
        $orderToProduct->select('otp.*', 'p.type', 'p.room_code', 'p.thumbnail_image_1', 'p.thumbnail_image_2', 'c.title as color_name', 'se.event_type', 'se.token', 'p.property_id', 'pr.name');
        $orderToProduct->where('otp.order_id', $order_id);
...
//dd($orderToProduct->get());
    ...
    public function getOrderData()
    {
        ...
        ->join('room_booked_date as rbd', 'rbd.order_id', '=', 'o.id')

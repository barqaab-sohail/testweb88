...
class ArticleController extends Controller
{
    ...
    public function store(ArticleCreateRequest $request)
    {
        ...
        // add meta description and keyword
        Article::where(['id' => $id])->update(['meta_descriptions' => $request->meta_description, 'meta_keyword' => $request->meta_keyword]);
    ...
    public function update(ArticleUpdateRequest $request)
    {
        ...
        if ($article) {
            ...
            if ($request->hasFile('front_image')) {
                // move the image to respective directories
                $input['front_image'] = time() . '.' . $request->front_image->getClientOriginalExtension();
                $request->front_image->move(storage_path('articles/front_image'), $input['front_image']);
            }

...
//        return response()->download($file_path,$filename,$headers );
    ...
    public function edit($id)
    {
		if(isset($_GET['type']) && $_GET['type'] = 2)
		{
			$enquiry = askForQota::find($id);
			$type = 2;
		}else{
			$enquiry = FreequoteEnquiry::find($id);
			$type = 1;
		}
        return view("admin.enquiry.edit", ['enquiry' => $enquiry, 'type' => $type]);
    }
    ...
    public function update($id, Request $request)
    {
		...
		if(isset($_REQUEST['type']) && $_REQUEST['type'] == 2)
		{
			return redirect()->to("/admin/support_tickets")->with('seu','true')->with('status', 'Service Enquiry updated successfully!')->with('type',$_REQUEST['type']);
		}elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 1) {
             return redirect()->to("/admin/support_tickets")->with('seu','true')->with('success', 'Service Enquiry updated successfully!')->with('type',$_REQUEST['type']);
        }
        else{
			//return redirect()->to("/admin/support_tickets")->with('status', 'Service Enquiry updated successfully!');	
            return redirect()->to("/admin/support_tickets")->with('seu','false')->with('success1', 'Service Enquiry updated successfully!');
		}

...
class ArticleController extends Controller
{
    public function index()
    {
        ...
        $grouped_articles = Article::groupBy(DB::raw("YEAR(post_date)"))->orderBy('post_date','desc')->get();
    ...
    public function show(Request $request, $id)
    {
        ...
        if (!empty($article)) {
            $title = $article->title;
            return view('frontend.blog.show', ['article' => $article, 'user' => $user, 'popular_post' => $popular_post, 'recent_post' => $recent_post, 'grouped_articles' => $grouped_articles, 'title' => $title]);
        }

...
class ShoppingCartControllerNew extends Controller
{
    public function index(Request $request)
    {
        ...
        // $services = "<b>Domain Registration:</b> <span class='sitecolor'>goodsetp.pk</span><br /><b>Domain Addons:</b><br/><i class='fa icon-arrow-right'>&nbsp;&nbsp;DNS Management (RM 133.00)<br/><i class='fa icon-arrow-right'>&nbsp;&nbsp;Name server management (RM 15.00)<br/>";
        //  $items->put('services', $services);

        $sum = $items->sum('price');
        $total =  number_format($sum, 2);
        $gstRate = GstRate::where('status', 1)->first()->rate;
        $gst = $sum * (float)$gstRate / 100;
        $grandTotal =  number_format(($sum + $gst), 2);
        ...
        if ($request->ajax()) {
            return DataTables::of($items)
                ...
                ->addColumn('services', function ($row) {
                    ...
                    $domainRegistration = "<b>Domain Registration:</b> <span class='sitecolor'>" . $row['id'] . "</span><br />";
                    ...
                    if ($row['plan_id']) {
                        ...
                        return "<div class='pull-left'>" . $services . "</div>";
                    } else {
                ...
                ->editColumn('cycle', function ($row) {
                    ...
                    return  "<div class='cycle_value'>" . $row['cycle'] . ' ' . $year . '<div class="pull-right"><a href="#" data-hover="tooltip"   class="editYear" data-id="' . $row['id'] . '" title="Edit">' . $editIcon . '</i></a></div>' .  "</div>";
                })
        ...
        return view('frontend.shopping_cart_new', compact('total'));
    }
    ...
    public function update_cycle(CartRequest $request)
    {
        $cart = session('cart');

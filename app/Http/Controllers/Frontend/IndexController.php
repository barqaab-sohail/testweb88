...
class IndexController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		return view("frontend.index", compact('indexplans', 'offer_services', 'page_cms', 'videos', 'testimonial', 'general_features', 'search_result', 'banners'));
	}
	...
	public static function get_hosting_categories()
	{
		$categories = Category::where('parent', 0)->where(['status' => 1, 'slug' => 'hosting'])->orderBy('sort_order')->get();
		foreach ($categories as $i) {
			if ($i->sub_categories) {
				$i->childs = $i->sub_categories->where('status', 1)->sortBy('sort_order')->chunk(3);
			}
	...
	public function client_dashboard()
	{
		...
		if ($user) {
			if ($user->role == 'Admin') {
				...
				$tickets = Tickets::where('user_id', $user->id)->orderBy('id', 'desc')->take(5)->get();
				//dd($tickets);
				//echo $user->id;
				$orders  = Order::where('user_id', $user->id)->orderBy('id', 'desc')->take(5)->get();
				...
				$unpaidOrders   = Order::where(['user_id' => Auth::id(), 'status' => 'INCOMPLETE'])->paginate(10);
				$domains = auth()->user()->domains;
				$expiredDomain = 0;
				foreach ($domains as $key => $domain) {
					if (strtotime($domain->exp_date) < strtotime('+30 days')) {
						$expiredDomain++;
					}
				}
				//dd($orders);
				return view("frontend.client_dashboard", [
					...
					'totalUnpaidOrdersAmount' => $totalUnpaidOrdersAmount,
	...
	public static function get_cart_items()
	{
		...
		if (session()->has('cart')) {
			if (count(session('cart')) > 0) {
				// Removing total_price and discount key while counting cart items.
				$total_cart = count(array_filter(session('cart'), function ($value, $key) {
					return !in_array($key, array('total_price', 'discount'));
	...
	{
		$results = DB::table('newsletter')->where('email', $request->email)->get();
		...
		if (count($results) != 0) {
			...
			$json['success'] = "Newsletter is subscribed";
			$json['status'] = 200;
			echo json_encode($json);
			exit;
		}

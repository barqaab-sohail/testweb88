...
class DomainConfigurationControllers extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $serverDetails     = $request->except(['_token']);
        $domain_availblity = [];
        $err               = 0;
        ...
        // print "<pre>";print_r($domain_availblity);exit("here");
        return view('frontend.domain_configuration_hosting', ['response' => $domain_availblity, 'err_preg' => $err, 'id' => $plan_id, 'server_details' => $serverDetails]);
    }
    ...
    private function singleSearch($domain)
    {
        ...
        if (!empty($bulk)) {
            ...
            $resp[0] = 0;
            if ($resp[0] == 0) {
                $success[$bulk] = ['status' => isset($resp[0]) ? $resp[0] : 0];
                $success[$bulk] = (object)$success[$bulk];
                $available = "$bulk";
            } else {
                $error[$bulk] = [
                    'status' => isset($resp[0]) ? $resp[0] :  ''
                ];

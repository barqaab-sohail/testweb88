...
class Promocodes extends Model{
	...
	{
		$promocodes = DB::table('promocodes as p')->select('p.*', DB::raw('(SELECT count(id) as total from plans WHERE plans.promocode_id = p.id) as timeOfUse'));
		// $promocodes = DB::table('promocodes as p')->select('p.*');
	...
	public function getPromocodeProducts($id){
		return DB::table('promocodes_to_product as ptp')->select('ptp.*', 'p.plan_name', 'p.service_code')->leftjoin('plans as p','p.id', '=','ptp.product_id' )->where('ptp.promocode_id', $id)->get();
	}
	...
	public function validPromoCode(){
		$countOfUsed = Plan::where('promocode_id',$this->id)->count();
